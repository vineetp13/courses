...
int con_overhead_client()
{
	...
	if( (PM_COUNT(connect(socket_desc, (struct sockaddr *) &server, sizeof(server)), "Connection Setup Time, "O_STR, 1)) < 0 )
	{
...
//	shutdown(socket_desc,SHUT_RDWR);
	PM_COUNT(close(socket_desc), "Connection Teardown Time, "O_STR,1);
	return 0;
}
...
//	PM_COUNT(con_overhead_client(), "Connection Overhead Time,  "O_STR, 10);
